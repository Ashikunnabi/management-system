...
function imageCapture(image_preview_id, file_id) {
    ...
    if (count == 1) {
        (function () {
            ...
            var width = 192;    // We will scale the photo width to this
    ...
    $('#save_captured_image').on('click', function () {
        if (default_image_data !== $('#photo').attr('src')){
            $('#'+image_preview_id+'').attr('src', $('#photo').attr('src'));
            $('#image_capture_close').click();
            ...
            //Usage example:
            var file = dataURLtoFile($('#photo').attr('src'),'hello.txt');
        }        
    });
}